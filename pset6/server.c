...
char* indexes(const char* path)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (name == NULL)
        {
            free(list);
            freedir(namelist, n);
            printf("Line 480"); error(500);
            return NULL;
        }
        
        // check for index.html or index.php
        if (strcmp(namelist[i]->d_name, "index.html") == 0)
        {
            new_path = realloc(new_path, strlen(path) + strlen(namelist[i]->d_name));
...
void interpret(const char* path, const char* query)
{
    ...
    {
        printf("Line 519"); error(500);
        return;
    }
    FILE* file = popen(command, "r");
    if (file == NULL)
    {
        printf("Line 525"); error(500);
        return;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    while((size = fread(&buf, 1, BYTES, file)) != 0)
    {
        //printf("Size: %zu\n", size);
        // append bytes to content 
        *content = realloc(*content, *length + size + 1);
        if (*content == NULL)
        {
            *length = 0;
            break;
        }
        memcpy(*content + *length, buf, size);
        *length += size;
        size = 0;
        //printf("Length: %zu\n", *length);
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    {
        method[new_index] = line[index];
        index++; new_index++;
    }
    method[new_index] = '\0';
    
    // Error Check: space after Method
    if (line[index] != ' ')
    {
        error(400);
        return false;
    }
    // Error Check: Method is GET
    if (strcmp(method, "GET") != 0)
    {
        error(405);
        return false;
    }
    // Advance index past space
    index++; new_index = 0;
    
    
    /*** Absolute Path ***/
    // Error Check: Absolute Path begins with '/'
    if (line[index] != '/')
    {
        error(501);
        return false;
    ...
    {
        // Error Check: no '"' marks
        if (line[index] == '"')
        {
            error(400);
            return false;
        }
        abs_path[new_index] = line[index];
        index++; new_index++;
    }
    abs_path[new_index] = '\0';
    ...
    /*** HTTP version ***/
    // Get HTTP Version
    while(line[index] != '\r' && line[index] != '\0')
    {
        http_version[new_index] = line[index];
        index++; new_index++;
    }
    http_version[new_index] = '\0';
    
    // Error Check: Version is HTTP/1.1
